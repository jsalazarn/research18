...
def FCK_fr(mean_perm, sd_perm, mean_phi, sd_phi, sizen, iteras, k_fr):
    """
    This function computes lognormal permeability considering a fracture
    permeability value [darcies]. It computes and displays the F and C capacities
    values and the (1-F)/F and (1-C)/C terms.
    ...
    for itera in np.arange(iteras):
        ...
        perm2 = np.random.random(*perm1.shape) * (k_fr - sd_perm) + (k_fr + sd_perm)
